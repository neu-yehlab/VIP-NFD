...
namespace nfd {
    namespace fw {
        namespace VIP{
NeighborEntry::NeighborEntry(const int face_id, const double vip_count, const double tx_vip_avg) : face_id(face_id), vip_count(vip_count), tx_vip_avg(tx_vip_avg) {}

bool NeighborEntry::operator==(const NeighborEntry& neighbor) {
    return (face_id == neighbor.face_id && vip_count == neighbor.vip_count && tx_vip_avg == neighbor.tx_vip_avg);
}
